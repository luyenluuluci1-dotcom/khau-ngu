...
document.addEventListener('DOMContentLoaded', () => {
    ...
    vocabItems.forEach(item => {
        item.addEventListener('click', () => {
            ...
            const shownElements = item.querySelectorAll('.pinyin:not(.hidden), .meaning:not(.hidden)');
    ...
    // Đăng ký phát âm cho Thanh mẫu & Vận mẫu
    ...
    vocabItemsInteractive.forEach(item => {
        ...
        item.addEventListener('click', (e) => {
            // Không can thiệp vào logic ẩn/hiện bính âm của phần 2, chỉ chạy song song phát âm
