// --- Script Điều Khiển Slide Bài Giảng Tương Tác Bài 4 ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    vocabItems.forEach(item => {
        item.addEventListener('click', (e) => {
            // Ngăn sự kiện nổi bọt gây kích hoạt phát âm trùng lặp nếu click trực tiếp vào chữ Hán
    ...
    const vocabItemsInteractive = document.querySelectorAll('.vocab-item');
    ...
    // Đăng ký phát âm cho các cụm từ quét mã so sánh
    const comparisonPhrases = document.querySelectorAll('.comparison-phrase');
    comparisonPhrases.forEach(phrase => {
        phrase.classList.add('playable');
        phrase.addEventListener('click', (e) => {
            e.stopPropagation();
            const zhText = phrase.querySelector('.zh');
            if (zhText) {
